...
__device__ __forceinline__ void VertexMapKinectProcess(cv::gpu::DevMem2D_<unsigned short> d, float *out, float *calib, int n, int m)
{
	...
	out[3*idx] = d(n-i-1,j) == 0 ? 0.0 : (calib[9]/calib[10]) * float(d(n-i-1,j)) * ((float(j)-calib[2])/calib[0]);
	out[3*idx+1] = d(n-i-1,j) == 0 ? 0.0 : (calib[9]/calib[10]) * float(d(n-i-1,j)) * ((float(i)-calib[3])/calib[1]);
	out[3*idx+2] = d(n-i-1,j) == 0 ? 0.0 : -(calib[9]/calib[10]) * float(d(n-i-1,j));
...
__device__ __forceinline__ void ReadProcess(float *VMap, float *RGBD, float *calib,  int n, int m) {
	...
	if (RGBD_v.w == 0.0 /*RGBD_v.x == 0.0 && RGBD_v.y == 0.0 && RGBD_v.z == 0.0*/) {
		VMap[3*idx] = 0.0;
		VMap[3*idx+1] = 0.0;
		VMap[3*idx+2] = 0.0;
		RGBD[4*idx+3] = 0.0;	
...
__device__ __forceinline__ void DiscontProcess (cv::gpu::DevMem2D_<unsigned char> mat, float *Vmap, int n, int m, float thresh) {
	int i = threadIdx.x + blockIdx.x * THREAD_SIZE_X;
    int j = threadIdx.y + blockIdx.y * THREAD_SIZE_Y;
	...
	float depth = Vmap[3*(i*m + j)+2];
	for (int k = bi; k < ui; k++) {
		for (int h = bj; h < uj; h++) {
			if (Vmap[3*(k*m + h)+2] !=0.0 && fabs(depth - Vmap[3*(k*m + h)+2]) > thresh) {
				mat(i,j) = 255;
				return;
			}
		}
...
__global__ void DiscontKernel (cv::gpu::DevMem2D_<unsigned char> mat, float *Vmap, int n, int m, float thresh) {
	DiscontProcess(mat, Vmap, n, m, thresh);
}
...
__device__ __forceinline__ void CpyProcessChar3 (cv::gpu::DevMem2D_<uchar[3]> mat, float *rgb, int n, int m) {
	...
	rgb[3*idx] =  float(mat(n-i-1, j)[2])/255.0;
...
__device__ __forceinline__ void ThreshProcess (cv::gpu::DevMem2D_<unsigned char> Out, cv::gpu::DevMem2D_<unsigned char> In, unsigned char thresh, unsigned char *Label, int n, int m) {
	int i = threadIdx.x + blockIdx.x * THREAD_SIZE_X;
    int j = threadIdx.y + blockIdx.y * THREAD_SIZE_Y;
    int idx = i*m + j;
	...
	Out(i,j) = (In(i,j) > thresh || Label[idx] > 0) ? 0 : 255;
}
__global__ void  ThreshKernel(cv::gpu::DevMem2D_<unsigned char> Out, cv::gpu::DevMem2D_<unsigned char> In, unsigned char thresh, unsigned char *Label, int n, int m) {
	ThreshProcess(Out, In, thresh, Label, n, m);
}
...
void gpu_Discont(cv::gpu::DevMem2D_<unsigned char> mat, float *VMap, int n, int m, float thresh) {
	dim3 dimBlock(THREAD_SIZE_X, THREAD_SIZE_Y);
	dim3 dimGrid (1, 1, 1);
	dimGrid.x = divUp (n, dimBlock.x);
	dimGrid.y = divUp (m, dimBlock.y);
	
    DiscontKernel<<<dimGrid, dimBlock>>>(mat, VMap, n, m, thresh);
...
void gpu_Thresh(cv::gpu::DevMem2D_<unsigned char> Out, cv::gpu::DevMem2D_<unsigned char> In, unsigned char thresh, unsigned char *Label, int n, int m) {
	dim3 dimBlock(THREAD_SIZE_X, THREAD_SIZE_Y);
	dim3 dimGrid (1, 1, 1);
	dimGrid.x = divUp (n, dimBlock.x);
	dimGrid.y = divUp (m, dimBlock.y);
	
    ThreshKernel<<<dimGrid, dimBlock>>>(Out, In, thresh, Label, n, m);
